...
void NoC::buildCommon()
{
	...
	// HG: Check for transaction table availability
...
void NoC::buildMesh()
{
    ...
    // HG: Adapted from Hub-based comm to be used by PEs
	...
	for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
		...
		// Add NetworkInterface to have Tile ID name
		t[i][j]->ni->local_id = tile_id;

	    // Check for traffic table availability
   		if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED)
		{
			// NetowrkInterface takes infor about traffic
			 t[i][j]->ni->traffic_table = &gttable;	// Needed to choose destination
	   		 t[i][j]->ni->never_transmit = (gttable.occurrencesAsSource(t[i][j]->pe->local_id) == 0);
		}
		// HG: Check availability of transaction table 
		...
		// HG: initial empty vector to pe_transaction_table
	...
	// ###### Fri Feb 7 20:53:40 MYT 2025
	// HG: Memory Tile instantiation
	for (int j = 0; j < GlobalParams::mesh_dim_y; j++) {
		// HG: Create the single Tile with a proper name
		...
		// HG: Tell the MemTile PE its coordinates
	...
	// HG: Originally is to clear signals on edge nodes, repurpose it for MemTile Connection
    for (int j = 0; j <= GlobalParams::mesh_dim_y; j++) {
	req[0][j].east = 0;
	ack[0][j].west = 0;
